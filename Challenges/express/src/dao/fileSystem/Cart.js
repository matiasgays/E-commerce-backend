...
class Cart {
  ...
  getCartById = async (cid) => {
    try {
      ...
      if (cartFound)
        return { error: 0, message: "cid found", payload: cartFound };
  ...
  addProductInCart = async (body) => {
    try {
      ...
      this.cart.forEach((element) => {
        if (element.id > this.maxId) this.maxId = element.id;
      });
    } catch (error) {
    ...
    if (cidFound) {
      ...
      if (pidFound) {
        ...
        try {
          ...
          return {
            error: 0,
            message: "Product is already in cart.  Product's quantity +1",
          };
        } catch (error) {
          throw new Error(error);
        }
